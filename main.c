...
const unsigned char Bunker0[] = {
 ...
 // small shield floating in space to cover the player's ship from enemy fire (decay image)
// width=18 x height=5
unsigned char Bunker1[] = {
 ,
...
STyp bunker1 = { (SCREENW-18)/4, SCREENH-10,18,5,0,{Bunker0,Bunker1}, 1, ALIVE };
STyp bunker2 = { 3*(SCREENW-18)/4, SCREENH-10,18,5,0,{Bunker0,Bunker1}, 1, ALIVE };
...
void DrawBunker()
{
	switch(bunker1.status)
	{
		case ALIVE:
			Nokia5110_PrintBMP( bunker1.x, bunker1.y, bunker1.image[0], 0 );
			break;
		default:
			Nokia5110_PrintBMP( bunker1.x, bunker1.y, bunker1.image[1], 0 );
			break;
	}
	switch(bunker2.status)
	{
		case ALIVE:
			Nokia5110_PrintBMP( bunker2.x, bunker2.y, bunker2.image[0], 0 );
			break;
		default:
			Nokia5110_PrintBMP( bunker2.x, bunker2.y, bunker2.image[1], 0 );
			break;
	}
}
...
// < 0 do not overlap, distance in pixels
// = 0 touch
// > 0 overlap size in pixel
...
unsigned char hasAlienCollided( STyp* const spr1 )
{
	...
	for(n=ALIEN_NUM-1; n>=0 ; --n)
		if(	alien[n].status == ALIVE )
			if( bitCollisionCheck( spr1, &alien[n]) )
				return n+1;
...
unsigned char hasMissileCollided( STyp* const s1 )
{
	...
	for(n=0; n < MAX_MISSILES; n++)
		if(	missile[n].status == ALIVE )
			if( bitCollisionCheck( s1, &missile[n]) )
				return n+1;
	return 0;
}

// 
void decayBunker( STyp* bunker, STyp* const s )
{
	unsigned char x,y;

	if( bunker->y < s->y ) {
		// s1 top of s2
		y = bunker->h - rectOverlapY(bunker, s);						// get sprites' overlap height
	} else {
		// s2 top of s1
		y = rectOverlapY(s, bunker);
	}
	if( bunker->x < s->x )
	{
		// s1 left of s2
		x = bunker->w - rectOverlapX(bunker, s);
	}	else {
		// s2 left of s1
		x = rectOverlapX(s, bunker);
	}
	
	setPixelBMP(x,y,(unsigned	char*)bunker->image[1],0);
	setPixelBMP(x-1,y,(unsigned	char*)bunker->image[1],0);
	setPixelBMP(x+1,y,(unsigned	char*)bunker->image[1],0);
	setPixelBMP(x,y+1,(unsigned	char*)bunker->image[1],0);
	setPixelBMP(x,y-1,(unsigned	char*)bunker->image[1],0);
	return;
}
...
void TimerRendering()
{
	...
	if( laser.status == ALIVE )
	{
		...
		// test collision with Bunker
		a_idx = bitCollisionCheck( &bunker1, &laser );
		if( a_idx )
		{
			laser.status = DEAD;											// flag laser as dead
			bunker1.status = DAMAGED;
			decayBunker( &bunker1, &laser );					// destroy banker part where missile fell
			Sound_Play( S_HIGHPITCH, SC_CHANNEL2 );
		}

		a_idx = bitCollisionCheck( &bunker2, &laser );
		if( a_idx )
		{
			laser.status = DEAD;											// flag laser as dead
			bunker2.status = DAMAGED;
			decayBunker( &bunker2, &laser );					// destroy banker part where missile fell
			Sound_Play( S_HIGHPITCH, SC_CHANNEL2 );
		}
	}
	...
	if( gl_m.maxMissiles )
	{
		...
		// test collision with Bunker
		a_idx = hasMissileCollided( &bunker1 );
		if( a_idx )
		{
			gl_m.maxMissiles--;												// reduce active missile count
			missile[a_idx-1].status = DEAD;						// kill missile
			bunker1.status = DAMAGED;
			decayBunker( &bunker1, &missile[a_idx-1] );	// destroy banker part where missile fell
			Sound_Play( S_HIGHPITCH, SC_CHANNEL2 );
		}
		a_idx = hasMissileCollided( &bunker2 );
		if( a_idx )
		{
			gl_m.maxMissiles--;												// reduce active missile count
			missile[a_idx-1].status = DEAD;						// kill missile
			bunker2.status = DAMAGED;
			decayBunker( &bunker2, &missile[a_idx-1] );	// destroy banker part where missile fell
			Sound_Play( S_HIGHPITCH, SC_CHANNEL2 );
		}
